...
class Auth
{
	...
	public function login($login, $password, $remember=FALSE)
	{
		...
		$selects = 'id, email, username, users.role_id, users.deleted, users.active, banned, ban_message, password_hash, password_iterations, force_password_reset,atasan';
		...
		if ($this->check_password($password, $user->password_hash))
		{
			...
			$this->setup_session($user->id, $user->username, $user->password_hash, $user->email, $user->role_id, $remember,'', $user->username, $user->atasan);
	...
	private function autologin()
	{
		...
		if ($query->num_rows() == 1)
		{
			...
			if ( ! $this->ci->session->userdata('user_id'))
			{
				...
				$this->ci->load->model('users/User_model', 'user_model');
				$user = $this->ci->user_model->select('id, username, email, password_hash, users.role_id,users.atasan')->find($user_id);

				if ( ! $user)
				{
					return;
				}
				$id_sub_bidang = isset($user->id_sub_bidang) ?  $user->id_sub_bidang : "";
				$this->setup_session($user->id, $user->username, $user->password_hash, $user->email, $user->role_id, TRUE, $test_token, $user->username, $user->atasan, $user->id_bidang, $id_sub_bidang);
			}
		}
