...
class Settings extends Admin_Controller
{
	...
	public function __construct()
    {
		...
		$subbidangs = $this->sub_bidang_model->find_all();
		Template::set('subbidangs', $subbidangs);
	...
	private function save_user($type='insert', $id=0, $meta_fields=array(), $cur_role_name = '')
	{
        ...
        if($this->input->post('atasan')!="")
			$data['atasan'] = $this->input->post('atasan');
		if($this->input->post('bidang')!="")
			$data['id_bidang'] = $this->input->post('bidang');
		if($this->input->post('id_subbid')!="")
			$data['id_subbid'] = $this->input->post('id_subbid');
	...
	public function getinfouser()
	{
		...
		$output = "";
		$pangkat = isset($datadetil['pangkat']) ? $datadetil['pangkat'] : '';
		$golongan = isset($datadetil['golongan']) ? $datadetil['golongan'] : '';
		$jabatan = isset($datadetil['jabatan']) ? $datadetil['jabatan'] : '';
		$json= array();
		$json['pangkat'] = $pangkat;
		$json['golongan'] = $golongan;
		$json['jabatan'] = $jabatan;
		$output .= $this->load->view('settings/infouser',array("datadetil"=>$datadetil),true);	
	...
	public function getdatadetil()
	{
		...
		$usersrec = $this->user_model->find($id_user);
		if( isset($usersrec) ) {
			$user = (array)$usersrec;
		}
		$id_bidang = isset($user['id_bidang']) ? $user['id_bidang'] : '';
		$json= array();
		$json['id_bidang'] = $id_bidang;
		echo json_encode($json); //display records in json format using json_encode
