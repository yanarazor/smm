...
class Contexts
{
	...
	public static function render_menuaccordion($mode='text', $order_by='normal',$mainmenu = "",$menuaktif = "", $top_level_only = FALSE)
	{
		self::$ci->benchmark->mark('context_menu_start');
		//die($mainmenu);
		$contexts = self::$contexts;
		...
		*/
		foreach ($contexts as $context)
		{
			if ( has_permission('Site.'. ucfirst($context) .'.View') == true || permission_exists('Site.'. ucfirst($context) .'.View') == false)
			{
				...
				$id = 'tb_'. $context;
				$classmenu = "subtitle";
				if(self::$ci->uri->segment(2)==$context)
					$classmenu = "active";
				...
				$nav .= "<li class='".$classmenu."'><a href='{$url}' class='action' id='{$id}' title='{$title}'>";
				$nav .= "<i class='icon-th'></i>&nbsp; ";
				//$nav .= '	<img src="#" width="16" height="16" alt="icon"/>';
				// Image
				if ($mode=='icon' || $mode=='both')
				{
					$nav .= "<img src='". Template::theme_url('images/context_'. $context .'.png') ."' alt='{$title}' />";
				...
				$nav .= "	<img src='". Template::theme_url('images/minus.png') ."' width='7' height='4' alt='arrow' class='arrow' />"; 
				$nav .= "</a>"; 
				if (!$top_level_only)
				{
					//die($menuaktif);
					$nav .= self::context_navacc($context,$mainmenu,$menuaktif);
				}
	...
	public static function render_menuaccordionnew($mode='text', $order_by='normal',$mainmenu = "",$menuaktif = "", $top_level_only = FALSE)
	{
		self::$ci->benchmark->mark('context_menu_start');
		//die($mainmenu);
		$contexts = self::$contexts;
		...
		*/
		foreach ($contexts as $context)
		{
			if ( has_permission('Site.'. ucfirst($context) .'.View') == true || permission_exists('Site.'. ucfirst($context) .'.View') == false)
			{
				...
				$class = check_class($context, true);
				$id = 'tb_'. $context;
				$classmenu = "mm-dropdown mm-dropdown-root";
				if(self::$ci->uri->segment(2)==$context)
					$classmenu = "mm-dropdown mm-dropdown-root open";
				...
				if ($mode=='text' || $mode=='both')
				{
					$nav .= '<span class="mm-text mmc-dropdown-delay animated fadeIn">'.$title.'</span>';
				}
...
public function context_navacc($context=NULL,$mainmenu,$menuaktif=NULL, $class='active', $ignore_ul=FALSE)
	{
		//die($mainmenu."INI");
		// Get a list of modules with a controller matching
		// $context ('content', 'settings', 'reports', or 'developer')
...
//			if ((isset($permissions[$context][$module]) && has_permission($permissions[$context][$module])) || (isset($permissions[$context]) && is_array($permissions[$context]) && !array_key_exists($module, $permissions[$context])))
		...
		$menu = self::build_sub_menuacc($context, $ignore_ul,$menuaktif);
	...
	public static function build_sub_menuacc($context, $ignore_ul=FALSE,$menuaktif="")
	{
		$list = '';
		$statusaktif = "";
		//DIE($menuaktif);
		...
		if (!$ignore_ul)
		{
			...
			$list = "<ul class='mmc-dropdown-delay animated fadeInLeft' ".$statusaktif.">\n";
		}
		...
		foreach (self::$menu as $topic_name => $topic)
		{
			...
			if (count($topic) > 1)
			{
				...
				foreach ($topic as $module => $vals)
				{
					$class = $menuaktif == self::$ci->uri->segment(3) ? ' class="mm-dropdown mm-dropdown-root active"' : '';
					//die($menuaktif);
					// If it has a sub-menu, echo out that menu only…
					if (isset($vals['menu_view']) && !empty($vals['menu_view']))
					{
	...
	{
		//echo strtolower(self::$ci->uri->segment(3));
		$class = "";
		if(strtolower($display_name)==strtolower(self::$ci->uri->segment(3)))
			$class="active";
		$item  = '<li><a href="'. site_url(self::$site_area .'/'. $context .'/'. $module) .'"';
		$item .= ' title="'. $title .'"><span class="mm-text">'. ucwords(str_replace('_', '', $display_name)) ."</span></a>\n";
		...
		//die(self::$ci->uri->segment(3));
		//echo strtolower($display_name.self::$ci->uri->segment(3)."<p/>");
		$class = $display_name == self::$ci->uri->segment(3) ? 'active' : 'active';
