...
class Sppd_jabodetabek_model extends BF_Model {
	...
	public function find_all($user="",$tanggal="")
	{
		...
		{
			$this->select($this->table_name .'.*,users.display_name,nama,kegiatan.judul as judul_kegiatan,GROUP_CONCAT(distinct(u.display_name) SEPARATOR "<br/>-") pengikut');
		}
		if($user!=""){
	...
	public function count_all($user="",$tanggal="")
	{
		if (empty($this->selects))
		{
			$this->select($this->table_name .'.*,users.display_name,nama');
		}
		if($user!=""){
			$this->db->where('users.display_name like "%'.$user.'%"');
		}
	...
	public function find_detil($id)
	{
		if (empty($this->selects))
		{
			$this->select($this->table_name .'.*,users.display_name,nama,nama_pejabat,pejabat_pemberi_perintah.nip as nippejabat');
		}
