	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
				...
				<div class='controls'>
					<select name="daftar_ptpp_ditujukan_kepada" id="daftar_ptpp_ditujukan_kepada" class="chosen-select-deselect">
						...
						<?php foreach($bidangs as $bidang):?>
							<option value="<?php echo $bidang->id?>" <?php if(isset($daftar_ptpp['id_bidang']))  echo  ($bidang->id==$daftar_ptpp['id_bidang']) ? "selected" : ""; ?>> <?php e(ucfirst($bidang->bidang)); ?></option>
							<?php endforeach;?>
						<?php endif;?>
					</select>
				...
				<div class='controls'>
					<select name="daftar_ptpp_diajukan_oleh" id="daftar_ptpp_diajukan_oleh" class="chosen-select-deselect">
						...
						<?php foreach($users as $user):?>
							<option value="<?php echo $user->id?>" <?php if(isset($daftar_ptpp['diajukan_oleh']))  echo  ($user->id==$daftar_ptpp['diajukan_oleh']) ? "selected" : ""; ?>> <?php e(ucfirst($user->display_name)); ?></option>
							<?php endforeach;?>
						<?php endif;?>
					</select>
			...
			<legend>Verifikasi</legend>
				<div class="control-group <?php echo form_error('daftar_ptpp_status_persetujuan') ? 'error' : ''; ?>">
					<?php echo form_label('Status &nbsp;', '', array('class' => 'control-label', 'id' => 'daftar_ptpp_status_persetujuan_label') ); ?>
					<div class='controls'> 
						<label class='radio' for='daftar_ptpp_status_persetujuan_option1'>
							<input id='daftar_ptpp_status_persetujuan_option1' name='daftar_ptpp_status_persetujuan' type='radio' value='1' <?php echo $daftar_ptpp['status_persetujuan']=="1" ? "checked" : "" ?> <?php echo set_radio('daftar_ptpp_status_persetujuan', '1'); ?> />
							Setuju
						</label>
						</br>
						<label class='radio' for='daftar_ptpp_status_persetujuan_option2'>
							<input id='daftar_ptpp_status_persetujuan_option2' name='daftar_ptpp_status_persetujuan' type='radio' value='0' <?php echo $daftar_ptpp['status_persetujuan']=="0" ? "checked" : "" ?> <?php echo set_radio('daftar_ptpp_status_persetujuan', '0'); ?> />
							Tidak Setuju
						</label>
							
							
						<span class='help-inline'><?php echo form_error('daftar_ptpp_status_persetujuan'); ?></span>
					</div>
				</div>
			</fieldset>
			<fieldset>
			...
			<div class="control-group <?php echo form_error('persetujuan') ? 'error' : ''; ?>">
					<?php echo form_label('Persetujuan Terhadap PKTP &nbsp;', '', array('class' => 'control-label', 'id' => 'persetujuan') ); ?>
					<div class='controls'> 
						<label class='radio' for='persetujuan1'>
							<input id='persetujuan1' name='persetujuan' type='radio' value='1' <?php echo $daftar_ptpp['persetujuan']=="1" ? "checked" : "" ?> <?php echo set_radio('persetujuan', '1'); ?> />
							Setuju
						</label>
						</br>
						<label class='radio' for='persetujuan2'>
							<input id='persetujuan2' name='persetujuan' type='radio' value='0' <?php echo $daftar_ptpp['persetujuan']=="0" ? "checked" : "" ?> <?php echo set_radio('persetujuan', '0'); ?> />
							Tidak Setuju
						</label>
...
-->
			<div class="control-group <?php echo form_error('tindakan_koreksi') ? 'error' : ''; ?>">
				<?php echo form_label('Koreksi', 'daftar_ptpp_tindakan_koreksi', array('class' => 'control-label') ); ?>
				<div class='controls'>
			...
			<div class="control-group <?php echo form_error('tindakan_korektif') ? 'error' : ''; ?>">
				<?php echo form_label('Tindakan Perbaikan', 'daftar_ptpp_tindakan_korektif', array('class' => 'control-label') ); ?>
				<div class='controls'>
...
-->
        	...
        	<legend>Lampiran/Bukti Fisik (.pdf)</legend>
              <div class="dropzone well well-sm foto_upload">
              </div>
             <div class="input-group divlampiran">
             	<div class='controls'>
				 <a href="<?php echo base_url().$this->settings_lib->item('site.urluploaded'); ?><?php echo $daftar_ptpp['lampiran']; ?>" target="_blank" class="fancy"><?php echo $daftar_ptpp['lampiran']; ?></a></div>
				</div>	  
			 </div>
			...
			<?php if ($this->auth->has_permission('Daftar_ptpp.Ptpp.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('daftar_ptpp_delete_confirm'))); ?>'); ">
...
<script type="text/javascript">
 ...
 Dropzone.autoDiscover = true;
    var foto_upload= new Dropzone(".dropzone",{
		 ...
		 url: "<?php echo base_url() ?>admin/ptpp/daftar_ptpp//saveberkas",
		 maxFilesize: 20,
		 parallelUploads : 1,
		 method:"post",
		 acceptedFiles:"application/pdf",
		 paramName:"userfile",
		 dictDefaultMessage:"<img src='<?php echo base_url(); ?>assets/images/dropico.png' width='50px'/><br>Drop Berkas/Lampiran Bukti Fisik disini atau klik area ini untuk browse file",
		...
		foto_upload.on("sending",function(a,b,c){
			 ...
			 c.append('id_log',<?php echo $id; ?>);
			 console.log('mengirim');           
		 });
		 foto_upload.on("success",function(a,b){
		 	//console.log(b);
           	var data_n=JSON.parse(b);
           	if(data_n.namafile != ''){
           		swal("Upload Lampiran selesai", "success");	
           	}else{
