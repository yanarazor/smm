	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
				...
				<div class='controls'>
					<select name="daftar_induk_rekaman_penanggung_jawab" id="daftar_induk_rekaman_penanggung_jawab" class="chosen-select-deselect">
						...
						<?php foreach($bidangs as $bidang):?>
							<option value="<?php echo $bidang->id?>" <?php if(isset($daftar_induk_rekaman['penanggung_jawab']))  echo  ($bidang->id==$daftar_induk_rekaman['penanggung_jawab']) ? "selected" : ""; ?>> <?php e(ucfirst($bidang->bidang)); ?></option>
							<?php endforeach;?>
						<?php endif;?>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('daftar_induk_rekaman_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/rekaman/daftar_induk_rekaman', lang('daftar_induk_rekaman_cancel'), 'class="btn btn-warning"'); ?>
				
			<?php if ($this->auth->has_permission('Daftar_induk_rekaman.Rekaman.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('daftar_induk_rekaman_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('daftar_induk_rekaman_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
