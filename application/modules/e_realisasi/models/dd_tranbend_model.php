...
class dd_tranbend_model extends BF_Model {

	protected $table_name	= "sasdd_tranbend";
	protected $key			= "id";
	...
	public function getrealisasiperkegiatan($kdgiat = "",$kdoutput = "",$kdsoutput = "",$kdkmpnen = "",$kdskmpnen="")
	{
		if (empty($this->selects))
		{
			$this->select('kdgiat,kdoutput,kdsoutput,kdkmpnen,kdskmpnen,sum(rupiah) as jumlah,kdgiat,kdoutput,kdsoutput');
		}
		if($kdkmpnen != ""){
			$this->db->where('trim(kdkmpnen)',$kdkmpnen);
		}
