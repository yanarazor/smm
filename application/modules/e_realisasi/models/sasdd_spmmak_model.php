...
class Sasdd_spmmak_model extends BF_Model {

	protected $table_name	= "sasdd_spmmak";
	protected $key			= "id";
	...
	public function getrealisasi($tahun = "",$kdkmpnen = "",$kdskmpnen="",$kdsoutput = "")
	{
		...
		{
			$this->select('kdgiat,kdoutput,sasdd_spmmak.kdkmpnen,sasdd_spmmak.kdskmpnen,sasdd_spmmak.kdakun,sum(nilmak) as jumlah,kdsoutput,trim(kdjenspm) as kdjenspm');
		}
		...
		//$this->db->where('(kdjenspm != "01" and kdjenspm != "02" and kdjenspm != "03" and kdjenspm != "07" and kdjenspm != "04")');
	...
	public function getrealisasilsperkegiatan($tahun = "")
	{
		if (empty($this->selects))
		{
			$this->select('kdgiat,kdoutput,kdsoutput,kdkmpnen,kdskmpnen,sum(nilmak) as jumlah');
		}
		$this->db->where('sasdd_spmmak.thang',$tahun);
	...
	public function getrealisasikegiatan($tahun = "")
	{
		if (empty($this->selects))
		{
			$this->select('kdgiat,kdoutput,kdsoutput,sum(nilmak) as jumlah');
		}
	...
	public function getrealisasiperbulan($tahun = "",$kdkmpnen = "",$kdskmpnen="",$kdsoutput = "")
	{
		if (empty($this->selects))
		{
			$this->select('YEAR(bf_sasdd_spmmak.tgspm) as year,MONTH(bf_sasdd_spmmak.tgspm) as month,sum(nilmak) as jumlah,kdgiat,kdoutput,sasdd_spmmak.kdkmpnen,sasdd_spmmak.kdskmpnen,sasdd_spmmak.kdakun,sum(nilmak) as jumlah,kdsoutput,trim(kdjenspm) as kdjenspm');
		}
		if($kdkmpnen != ""){
			$this->db->where('trim(kdkmpnen)',$kdkmpnen);
		}
	...
	public function getrealisasilsold($tahun = "",$kdkmpnen = "",$kdskmpnen="",$kdsoutput = "")
	{
		if (empty($this->selects))
		{
	...
	public function rekapperkdakun($tahun = "",$kdakun = "")
	{
		if (empty($this->selects))
		{
			$this->select('sum(nilmak) as jumlah');
		}
		
		if($kdakun != ""){
			$this->db->where('kdakun like "'.$kdakun.'%"');
		}
		if($tahun != ""){
