	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tbody>
				...
				?>
				<tr <?php if($record->status == "4") { ?> class="rowred" <?php } ?>>
					<?php if ($can_delete) : ?>
					<td class="column-check"><input type="checkbox" name="checked[]" value="<?php echo $record->id; ?>" /></td>
					<?php endif;?>
					
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/perbaikan/perbaikan_sarpras/perbaikanedit/' . $record->id, '<span class="icon-pencil"></span>' .  $record->nomor); ?></td>
				<?php else : ?>
