	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tfoot>
				...
				<tr>
					<td colspan="<?php echo $num_columns; ?>">
						...
						<input type="submit" name="delete" id="delete-me" class="btn btn-danger" value="<?php echo lang('bf_action_delete'); ?>" onclick="return confirm('<?php e(js_escape(lang('perbaikan_sarpras_delete_confirm'))); ?>')" />
					</td>
				</tr>
				<?php endif; ?>
			</tfoot>
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/perbaikan/perbaikan_sarpras/periksa/' . $record->id, '<span class="icon-pencil"></span>' .  $record->nomor); ?></td>
				<?php else : ?>
				...
				<?php endif; ?>
					...
					<td><?php e($record->keluhan) ?></td>
					<td><?php e($record->display_name) ?></td>
					<td><?php e($record->tanggal_kirim) ?></td>
					<td><?php echo $record->catatan_kpu; ?></td>
					<td><?php echo number_format((double)$record->biaya_kalibrasi); ?></td>
					<td><?php echo $record->rekomendasi; ?></td>
					<td><span class='label label-warning'><?php e($record->status_perbaikan) ?></span></td>
				</tr>
				<?php
					endforeach;
				else:
