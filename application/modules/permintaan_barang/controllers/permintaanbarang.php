...
class permintaanbarang extends Admin_Controller
{
	...
	public function periksa()
	{
		...
		*/
		...
		$total = $this->permintaan_barang_model->count_pj_all($keyword,$bulan,$tahun,$kg,$status,$this->current_user->id);
		...
		*/
		//$this->permintaan_barang_model->where('status_permintaan',"1");
		$records = $this->permintaan_barang_model->limit($limit, $offset)->find_pj_all($keyword,$bulan,$tahun,$kg,$status,$this->current_user->id);
		if(isset($records) && is_array($records) && count($records) && $total>0)
			$total = $total;
		else
			$total = 0;
	...
	public function ppk()
	{
		...
		*/
		$total = $this->permintaan_barang_model->count_ppk_all($keyword,"","",$kg,$status,$pg,$this->current_user->id);
		...
		*/
		$records = $this->permintaan_barang_model->limit($limit, $offset)->find_ppk_all($keyword,"","",$kg,$status,$pg,$this->current_user->id);
		if(isset($records) && is_array($records) && count($records) && $total>0)
			$total = $total;
		else
			$total = 0;
	...
	public function verpj()
	{
		...
		if (isset($_POST['save']))
		{
			...
			if ($this->save_verifikasiatasan('3','update', $id))// perubahan sekarang setelah dari pj kegiatan langsung ke perssediaan atau BMN status 3 awal status 2
			{ 
				// save to log permintaan
				$keterangan = "";
				if($this->input->post('status_atasan') == "1")
				{
					...
					$this->save_log($id,"","Verifikasi PJ Kegiatan",$keterangan);
				...
				// Log the activity
				log_activity($this->current_user->id, "Verifikasi atasan langsung/PJ Kegiatan " .': '. $id .' : '. $this->input->ip_address(), 'permintaan_barang');
