...
class permintaanbarang extends Admin_Controller
{
	...
	public function bmn()
	{
		...
		//$this->permintaan_barang_model->where('status_permintaan >= 3');
		$total = count($this->permintaan_barang_model->find_bmn($keyword,$bulan,$tahun,$kg));
		...
		//$this->permintaan_barang_model->where('status_permintaan >= 3');
		$records = $this->permintaan_barang_model->limit($limit, $offset)->find_bmn($keyword,$bulan,$tahun,$kg);
		if(isset($records) && is_array($records) && count($records) && $total>0)
			$total = $total;
		else
			$total = 0;
	...
	public function pengadaan()
	{
		...
		$total = count($this->permintaan_barang_detil_model->find_pengadaan($keyword,$bulan,$tahun,$kg));
		...
		$this->pagination->initialize($this->pager);	
		$this->permintaan_barang_detil_model->where('(status_pengadaan = "1" or status_pengadaan = "2" or status_pengadaan = "5")');
		$records = $this->permintaan_barang_detil_model->limit($limit, $offset)->find_pengadaan($keyword,$bulan,$tahun,$kg);
		if(isset($records) && is_array($records) && count($records) && $total>0)
			$total = $total;
		else
			$total = 0;
	...
	public function createpengadaan()
	{
		...
		if (isset($_POST['save']))
		{
			...
			if ($this->edit_detil_barang_topengadaan($jumlah,$stat,"1","",date("Y-m-d"),'update', $kode))
			{
