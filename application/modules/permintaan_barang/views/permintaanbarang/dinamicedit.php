	...
	<tr id="divbarang_<?=$index?>"> 
		...
		<td width="30px" height="40px">
			<select id='mark_<?=$index?>' class="validate[required]" name="mark[]" style="width:80px">
				  ...
				  <?php if (isset($recordtmak) && is_array($recordtmak) && count($recordtmak)):?>
				  <?php foreach($recordtmak as $record):?>
					   <option value="<?php echo $record->kdmak;?>"><?php echo $record->nmmak; ?></option>
				   <?php endforeach;?>
				  <?php endif;?>
			  </select>
		...
		<td width="250px">
			<textarea id='spek_<?=$index?>' class="validate[required]"  type='text' name='spek[]' rows="2" style="width:200px"></textarea> 
		  </td>
		  <td width="20px">
			 <input id='jumlah_<?=$index?>' onchange="hitungjumlah('<?=$index?>')" type='text' name='jumlah[]' maxlength="10" width="20px"  style="width:20px"/>
		  </td>
		  <td width="300px">
		  	<select id='satuan_<?=$index?>' class="validate[required]" name="satuan[]" style="width:80px">
				  <option value="">Pilih</option>
		  ...
		  <td width="200px">
			 <input id='harga_<?=$index?>'  class="auto"  onchange="hitungjumlah('<?=$index?>')" type='text' name='harga[]' style="width:100px" maxlength="10"/>
		  </td>
		  <td width="200px">
			 <input id='jumlah_all_<?=$index?>'  class="auto"  type='text' name='jumlah_all[]' maxlength="10" style="width:100px"/>
		  </td>
		  <td width="200px"> 
			 <input type="file" name="file_upload_<?=$index?>" title="Pilih File"/>
		  </td>
		   <td> 
			 #
		  </td>
