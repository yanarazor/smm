...
$can_edit		= $this->auth->has_permission('Permintaan_Barang.Permintaanbarang.btu');
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tbody>
				...
				?>
				<tr <?php if($record->status_permintaan == "2") { ?> class="rowred" <?php } ?>>
					<?php if ($can_delete) : ?>
					<td class="column-check"><input type="checkbox" name="checked[]" value="<?php echo $record->id; ?>" /></td>
					<?php endif;?>
					
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/permintaanbarang/permintaan_barang/verkb/' . $record->id, '<span class="icon-pencil"></span>' .  $record->nomor); ?></td>
				<?php else : ?>
