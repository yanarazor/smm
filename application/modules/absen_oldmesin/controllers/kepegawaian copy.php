			...
			foreach ($recordabsen as $record) :
				...
				if($record->model=="Scan Keluar" and (!isset($dataabsen['P_'.$record->tanggal."_".$record->nik]) or isset($arddk['P_'.$record->tanggal."_".$record->nik])))
				{
					...
					}else
					{
						...
						//cek pulang
						$jamdatang = isset($dataabsen['M_'.$record->tanggal."_".$record->nik]) ? $dataabsen['M_'.$record->tanggal."_".$record->nik] : "07:30:00";
						//$datang1 = strtotime($record->tanggal ." 07:30:00");
						$datang1 = strtotime($record->tanggal ." ".$jamtoleransi); // jam pulang + toleransi
						$datang2 = strtotime($record->tanggal." ".$jamdatang);
						if($datang2>$datang1)
							$intervaldatang  = abs($datang2 - $datang1);
						else
							$intervaldatang = 0; 
						
						//$minutesdatang   = round($intervaldatang / 60); // jumlah meni kedatangan telatnya dari jam 7:30
						//echo $minutesdatang."<br>";
						
						$minutes   = 0;
						
						//echo $dataabsen['P_'.$record->tanggal."_".$record->nik]." data<br>";
						// hitung pulang cepat
						$datetime1 = strtotime($record->tanggal ." ".$jamplg);
						//$datetime1 = strtotime($record->tanggal ." 16:00:00");
						$datetime2 = strtotime($record->tanggal." ".$record->jam);
						if($datetime2 < $datetime1)
			...
			foreach ($recordabsen as $record) :
				...
				if($record->model=="Scan Keluar" and (!isset($dataabsen['P_'.$record->tanggal."_".$record->nik]) or isset($arddk['P_'.$record->tanggal."_".$record->nik])))
				{
					...
					}else
					{
						...
						//cek datang
						$jamdatang = isset($dataabsen['M_'.$record->tanggal."_".$record->nik]) ? $dataabsen['M_'.$record->tanggal."_".$record->nik] : "07:30:00";
						$datang1 = strtotime($record->tanggal ." ".$jamtoleransi);
						$datang2 = strtotime($record->tanggal." ".$jamdatang);
						if($datang2>$datang1)
							$intervaldatang  = abs($datang2 - $datang1);
						else
