		...
		<fieldset>
			...
			<div class="control-group <?php echo form_error('kesesuaian') ? 'error' : ''; ?>">
				...
				<div class='controls' aria-labelled-by='daftar_periksa_audit_kesesuaian_label'>
						...
						<input id='daftar_periksa_audit_kesesuaian_option1' name='daftar_periksa_audit_kesesuaian' type='radio' <?php if(isset($daftar_periksa_audit['kesesuaian'])) { if($daftar_periksa_audit['kesesuaian']== "1")  echo "checked"; }?> class='' value='1' />
						Sesuai
				 
                    <br />
					 
						<input id='daftar_periksa_audit_kesesuaian_option2' name='daftar_periksa_audit_kesesuaian' type='radio' <?php if(isset($daftar_periksa_audit['kesesuaian'])) { if($daftar_periksa_audit['kesesuaian']== "0")  echo "checked"; }?> class='' value='0' />
						Tidak
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('daftar_periksa_audit_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/audit/daftar_periksa_audit', lang('daftar_periksa_audit_cancel'), 'class="btn btn-warning"'); ?>
				
			<?php if ($this->auth->has_permission('Daftar_Periksa_Audit.Audit.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('daftar_periksa_audit_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('daftar_periksa_audit_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
