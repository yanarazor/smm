...
class kepegawaian extends Admin_Controller
{
	...
	public function rekap_content()
	{
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12"){
			...
			$recordsuratizin  	= $this->surat_izin_model->find_rekap($bulan,$tahun,"",$nama);
		}else{
		...
		if(isset($recordsuratizin) && is_array($recordsuratizin) && count($recordsuratizin)):
			foreach ($recordsuratizin as $record) :
				...
				if($record->tanggal != "" and $record->tanggal_selesai != "" and $record->tanggal != "0000-00-00" and $record->tanggal_selesai != "0000-00-00" and ($record->tanggal !=$record->tanggal_selesai))
				{
					...
					for($x = 0; $x < $arrlength; $x++) {
						if($record->izin=="1"){ // izin tidak masuk
							...
							{
								$dataabsen['M_'.$tanggaldarisampai[$x]."_".$record->nip] = "CT";
								$dataabsen['P_'.$tanggaldarisampai[$x]."_".$record->nip] = "CT";
					
								$dataizin['izin_'.$tanggaldarisampai[$x]."_".$record->nip] = "CT";
								$dataizin['izinp_'.$tanggaldarisampai[$x]."_".$record->nip] = "CT";
				...
				if($record->izin=="1"){ // izin tidak masuk
					$dataabsen['M_'.$record->tanggal."_".$record->nip] = "CT";
					$dataabsen['P_'.$record->tanggal."_".$record->nip] = "CT";
					
					$dataizin['izin_'.$record->tanggal."_".$record->nip] = "CT";
					$dataizin['izinp_'.$record->tanggal."_".$record->nip] = "CT";
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12"){
			$this->sppd_jabodetabek_model->where('pegawai',$nip);
		}
		...
		if(isset($recordsppd_jabodetabek) && is_array($recordsppd_jabodetabek) && count($recordsppd_jabodetabek)):
			foreach ($recordsppd_jabodetabek as $record) :
					$tanggaldarisampai =  $this->returnBetweenDates($record->tanggal_berangkat, $record->sampai_tanggal);
					$arrlength = count($tanggaldarisampai);
					//echo $arrlength."<br>";
					for($x = 0; $x < $arrlength; $x++) {
						$hari = date('l', strtotime($tanggaldarisampai[$x]));
						if($hari != "Saturday" and $hari != "Sunday")  // jika bukan sabtu minggu
						{
							//echo $tanggaldarisampai[0]."<br>";
							$dataabsen['M_'.$tanggaldarisampai[$x]."_".$record->nip] = "DDK"; 
							$dataabsen['P_'.$tanggaldarisampai[$x]."_".$record->nip] = "DDK"; 
							$arddk['M_'.$tanggaldarisampai[$x]."_".$record->nip] = "DDK"; 
							$arddk['P_'.$tanggaldarisampai[$x]."_".$record->nip] = "DDK"; 
						}
					}
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12"){
			$this->sppd_model->where('nip',$this->current_user->id);
			$recordsppd  	= $this->sppd_model->find_rekap($bulan,$tahun,"",$nama);
		}else{
			$recordsppd  	= $this->sppd_model->find_rekap($bulan,$tahun,$nip,$nama);
		}
		if(isset($recordsppd) && is_array($recordsppd) && count($recordsppd)):
			foreach ($recordsppd as $record) :
					//$datasppd['TL_'.$record->tanggal_berangkat."_".$record->nip] = "TL";
					$dataabsen['M_'.$record->tanggal_berangkat."_".$record->nip] = "DL"; 
					$dataabsen['P_'.$record->tanggal_berangkat."_".$record->nip] = "DL"; 
					$arddk['M_'.$record->tanggal_berangkat."_".$record->nip] = "DL"; 
					$arddk['P_'.$record->tanggal_berangkat."_".$record->nip] = "DL"; 
					//echo "Jabodetabek  ". $record->tanggal_berangkat."_".$record->nip;
					//echo 'P_'.$record->tanggal_berangkat."_".$record->nip." data<br>";
			endforeach;
		endif;
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12")
		{
			...
			$recordsppd_pengikut  	= $this->sppd_pengikut_model->find_rekap($bulan,$tahun,"",$nama);
		}else{
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12")
		{
			...
			$recordlupatimer  	= $this->lupa_timer_model->find_rekap($bulan,$tahun,"",$nama);
		}else{
		...
		if(isset($recordlupatimer) && is_array($recordlupatimer) && count($recordlupatimer)):
			foreach ($recordlupatimer as $record) :
				if($record->absen=="Pulang"){
					...
					$dataabsen['P_'.$record->tanggal_absen."_".$record->nip] = $record->jam_sebenarnya; 
				}else{
					//echo $record->jam_sebenarnya;
					$dataizin['izinMt_'.$record->tanggal_absen."_".$record->nip] = substr($record->jam_sebenarnya,0, 5);
					$dataabsen['M_'.$record->tanggal_absen."_".$record->nip] = $record->jam_sebenarnya; 
				}
			endforeach;
		endif;
		// end Lupa timer
		...
		if(isset($recordabsen) && is_array($recordabsen) && count($recordabsen)):
			foreach ($recordabsen as $record) :
				...
				//}
				$modelscan = $record->model;
				// cek jika bukan scan masuk dan bukan scan keluar
				if($modelscan != "Scan Masuk" and $modelscan != "Scan Keluar"){
					$akhirmasuk = "12:00:00";
					$mulaikeluar = "12:00:01";
					$timemasuk = strtotime($record->tanggal ." ".$akhirmasuk);
					$timekeluar = strtotime($record->tanggal ." ".$mulaikeluar);
					$realtime = strtotime($record->tanggal." ".$record->jam);
					if(($timemasuk > $realtime) and (!isset($dataabsen['M_'.$record->tanggal."_".$record->nik])))
					{
						$modelscan = "Scan Masuk";
					}
					if(($timekeluar < $realtime) and (!isset($dataabsen['P_'.$record->tanggal."_".$record->nik])))
					{
						$modelscan = "Scan Keluar";
					}
				}
				if($modelscan == "Scan Masuk" and (!isset($dataabsen['M_'.$record->tanggal."_".$record->nik]) or isset($arddk['M_'.$record->tanggal."_".$record->nik]))){
					//echo 'Masuk M_'.$record->tanggal." ".$record->jam." ".$record->nik."<br>";
				...
				//echo 'Pa_'.$record->tanggal."_".$record->nik."_".$record->model." data<br>";
				if($modelscan == "Scan Keluar" and (!isset($dataabsen['P_'.$record->tanggal."_".$record->nik]) or isset($arddk['P_'.$record->tanggal."_".$record->nik])))
				{
...
*/
	...
	public function downloadlist()
	{
		...
		if($this->current_user->role_id != "1" and $this->current_user->role_id != "16" and $this->current_user->role_id != "20" and $this->current_user->role_id != "12")
			$this->absen_model->where('nik',$this->current_user->nip);
		$records = $this->absen_model->find_all($nip,$nama,$tgl,$bulan,$tahun);
		...
		$objPHPExcel = PHPExcel_IOFactory::load($this->settings_lib->item('site.pathuploaded').'kosong.xls');
		$objPHPExcel->setActiveSheetIndex(0);
		$jumlah = 0;
		
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0,1,"Nik");
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1,1,"Nama");
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2,1,"Tanggal");
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3,1,"Jam");
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4,1,"Verifikasi");
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5,1,"Model");
		if (isset($records) && is_array($records) && count($records)) :
			foreach ($records as $record) :
				$weekday = date('l', strtotime($record->tanggal)); // note: first arg to date() is lower-case L
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0,$row,$record->nik);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1,$row,$record->nama);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2,$row,$convert->getday($weekday));
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3,$row,$convert->fmtDate($record->tanggal,"dd month yyyy"));
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4,$row,$record->jam);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5,$row,$record->verifikasi);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6,$row,$record->model);
			$row++;
			endforeach;
		endif;
		  
		$filename = "absen".mt_rand(1,100000).'.xls'; //just some random filename
		header('Content-Type: application/vnd.ms-excel');
		header('Content-Disposition: attachment;filename="'.$filename.'"');
		header('Cache-Control: max-age=0');
