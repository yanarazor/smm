	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal" id="frminput"'); ?>
		<fieldset>
				...
				<div class='controls'>
					<select class="text-input" name="pegawai_golongan" id="pegawai_golongan" class="chosen-select-deselect" style="width:700px">
						...
						<?php foreach($golongans as $rec):?>
							<option value="<?php echo $rec->id?>" <?php if(isset($pegawai['golongan']))  echo  ($rec->id==$pegawai['golongan']) ? "selected" : ""; ?>> <?php e(ucfirst($rec->pangkat)); ?></option>
							<?php endforeach;?>
						<?php endif;?>
					</select>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('pegawai_action_create'); ?>"  />
				<?php echo lang('bf_or'); ?>
